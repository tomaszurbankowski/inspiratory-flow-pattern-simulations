...
C) Corresponding cumulative compartmental airway-opening energies,
   global airway-opening energy E_aw = E_1 + E_2, and the Partition Index
   PI = E_2 / E_aw.
...
def make_panel_c(ax, vent, res) -> None:
    ...
    # Global inspiratory energy/work at the airway opening.
    energy_aw = energy1 + energy2
    pi = energy2[-1] / energy_aw[-1]

    ax.plot(t, energy_aw, color=COL_FTOT, lw=LW_MAIN)
    ...
    ax.set_xlim(0, t[-1])
    ax.set_xticks(np.arange(0, 1.01, 0.2))
    ax.set_ylim(0, energy_aw[-1] * 1.18)
